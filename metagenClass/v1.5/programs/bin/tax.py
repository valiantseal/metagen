...
def main():
    # Set the directory where the sample folders are located
    process_dir = './process'
    ...
    for sample_dir in os.listdir(process_dir):
        ...
        # Define the paths to the BLAST and Kraken result files
        blast_file = os.path.join(sample_path, 'blastResTop_3.tsv')
        kraken_file = os.path.join(sample_path, 'krakenSelVirReads.tsv')
        
        # Read the files into DataFrames, ensuring proper handling of tabs and quotes
        blast_df = pd.read_csv(blast_file, sep='\t', quotechar='"')
        kraken_df = pd.read_csv(kraken_file, sep=',', quotechar='"')  # Change separator to comma

        # Merge the BLAST and Kraken DataFrames on the 'Read' column
        merged_df = pd.merge(blast_df, kraken_df, on='Read', how='outer')

        # Add a 'Sample' column to the merged DataFrame
