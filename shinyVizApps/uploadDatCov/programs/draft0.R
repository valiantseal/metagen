                   ...
                   tabPanel("Summary Stat", fluid=T,
                            fluidPage(
                              fileInput('target_upload', 'Upload Your Metadata',
                                        accept = c('.tsv', '.txt')),
                              tags$div(title="Input column that contains a unqiue ID for each sample",
                                textInput(inputId = "uuid",  label="ID column", value = "strain")),
                              linebreaks(2),
                   ...
                   tabPanel("Phylogeny", fluid=T,
                            fluidPage(
                              ...
                              splitLayout(cellWidths=c("50%", "50%"), 
                                          tags$div(title= "Subsample/highlight tree by the input value.Input value should be in 
                                                   the column selected above",
...
server <- function(input, output) {
  ...
  uuid<-reactive({
    selCol<-as.character(input$uuid)
    return(selCol)
  })
