...
cds <- align_cds(cds, num_dim = 75, alignment_group = "group")
cds <- reduce_dimension(cds)
conStr<-plot_cells(cds, color_cells_by="group", label_cell_groups=FALSE, cell_size=1)

#ggsave(paste0('OPC_ODC_MonocClust_75PC_Root2_StressVsContr', curDate, '.jpeg'), plot = conStr, height = 12, width = 12, units = 'in', dpi = 300)
...
ggsave(paste0('OPC_ODC_MonocClust_75PC_Root2_ColClust', curDate, '.jpeg'), plot = plotClust, height = 12, width = 12, units = 'in', dpi = 300)
...
ggsave(paste0('OPC_ODC_MonocClust_75PC_Root2_', curDate, '.jpeg'), plot = psedPlot, height = 12, width = 12, units = 'in', dpi = 300)
ggsave(paste0('OPC_ODC_MonocClust_50PC_Root2_Box_', curDate, '.jpeg'), plot = psedBox, height = 12, width = 12, units = 'in', dpi = 300)
write.csv(medTimes, paste0('OPC_ODC_MonocClust_50PC_Root2_sum_', curDate, '.csv'), row.names = F)

monocUmap<-plot_cells(cds, label_branch_points = F,label_roots = F, cell_size=1, group_label_size = 4)+
  theme(text = element_text(size = 20)) 
ggsave(paste0('OPC_ODC_MonocClust_UMAP_', curDate, '.jpeg'), plot = monocUmap, height = 12, width = 12, units = 'in', dpi = 300)
...
oldLabel<-plot_cells(cds, color_cells_by="integrated_snn_res.0.2", label_roots = F, label_leaves = F, group_label_size = 4, cell_size=1)+
  theme(text = element_text(size = 20)) 
ggsave(paste0('OPC_ODC_MonocClust_UMAP_OldLabels', curDate, '.jpeg'), plot = oldLabel, height = 12, width = 12, units = 'in', dpi = 300)
...
write.csv(marker_test_res, paste0('OPC_ODC_MonocClust_75PC_clusterMarkersTop100_', curDate, '.csv'), row.names = F)
write.csv(marker_test_group, paste0('OPC_ODC_MonocClust_75PC_groupMarkersTop100_', curDate, '.csv'), row.names = F)
...
gene_fits <- fit_models(cds, model_formula_str = "~monocle3_pseudotime")
fit_coefs <- coefficient_table(gene_fits)
coefFiltr<-data.frame(fit_coefs[(fit_coefs$status == 'OK') & (fit_coefs$term == 'monocle3_pseudotime'),])
coefFiltr<-coefFiltr[, c(1:3, 6:ncol(coefFiltr))]

write.csv(coefFiltr, paste0('OPC_ODC_MonocClust_75PC_top100G_TimeCor_', curDate, '.csv'), row.names = F)
# take group into account
genes_adjGroup <- fit_models(cds, model_formula_str = "~monocle3_pseudotime + group")
...
coefAdjGrFiltr<-coefAdjGrFiltr[, c(1:3, 6:ncol(coefAdjGrFiltr))]
write.csv(coefAdjGrFiltr, paste0('OPC_ODC_MonocClust_75PC_top100G_adjGr_TimeCor_', curDate, '.csv'), row.names = F)
# compare models
modelComp<-compare_models(gene_fits, genes_adjGroup)


time_batch_models <- fit_models(cds,
                                model_formula_str = "~monocle3_pseudotime + group",
                                expression_family="negbinomial")
time_models <- fit_models(cds,
                          model_formula_str = "~monocle3_pseudotime",
                          expression_family="negbinomial")
compModel<compare_models(time_batch_models, time_models) # quasi poison models cannot be compares, so use negative binomial

write.csv(compModel, paste0('OPC_ODC_MonocClust_75PC_top100G_TimeCor_ModelComp_', curDate, '.csv'), row.names = F)