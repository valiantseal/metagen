...
getConsensus <- function(metaSeq, ampSeq, protocol, maxFreq, minFreq, freqCol) {
  ...
  if (protocol == "ampseq") {
    ...
    targDf = ampseq[ampseq$Samp_Pos_Ref_Alt%in%selSnps,]
    
    # data to check agains
    refDf = metaseq[metaseq$Samp_Pos_Ref_Alt%in%selSnps,]
    targSnv <- unique(refDf$Samp_Pos_Ref_Alt)
  } else if (protocol == "metaseq") {
    # dataframe with stats
    targDf = metaseq[metaseq$Samp_Pos_Ref_Alt%in%selSnps,]
    
    # data to check agains
    refDf = ampseq[ampseq$Samp_Pos_Ref_Alt%in%selSnps,]
    targSnv <- unique(refDf$Samp_Pos_Ref_Alt)
  }
