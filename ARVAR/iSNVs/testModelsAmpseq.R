...
dfFilt = dfFilt[dfFilt$ALLELE.FREQUENCY <=1,]
...
aucModel = glm(ConsTest ~ ALLELE.FREQUENCY + STRAND.BIAS + DEPTH + 
                 I(DEPTH^2) + QUAL + Var_Al_RelPos + I(Var_Al_RelPos^2) + 
...
# vif testing
vif_model2 <- glm(ConsTest ~ ALLELE.FREQUENCY  + 
                    QUAL + Var_Al_RelPos   + Mean_depth , data = dfFilt, family = "binomial")
print(vif(vif_model2))


set.seed(42)
aucModel = glm(ConsTest ~ ALLELE.FREQUENCY  + 
...
finalModel = glm(ConsTest ~ ALLELE.FREQUENCY  + 
...
write.csv(modelSum , "model_summary/ampseq_metaseq_97_freq_0.02-0.98_lin_sum.csv", row.names = F)
#write.csv(final_vif, "model_summary/metaseq_ampseq_97_freq_0.02-0.98_VIF.csv", row.names = F)

png(file = "model_summary/ampseq_metaseq_97_freq_0.02-0.98_lin_auc.png" ,height = 10, width = 10, units = "in", res = 300)
