...
getRes = function(path, pat) {
  ...
  for ( i in filesList) {
    ...
    df = read.csv(inFile)
    dfFilt = df[(df$FILTER == "PASS") & (df$ALLELE.FREQUENCY >= 0.02),]
    dfFilt$Sample = pat
...
ampSnvs = unique(getConsAllSamp(samplesList = samplesList, path = "process/"))
length(ampSnvs)

metaSnvs = unique(getConsAllSamp(samplesList = samplesList, path = "288_metaseq/process/"))
length(unique(metaSnvs))

allConsSnv = unique(metaSnvs[(metaSnvs%in%ampSnvs)])
length(allConsSnv)

failSnvs = unique(metaSnvs[!(metaSnvs%in%ampSnvs)])
length(failSnvs)

# get combined table of all the metaseq results
...
metaResDf = getAllResBind(samplesList = samplesList, path = "288_metaseq/process/")
...
#write.csv(metaResDf, file = "test_consensus/vivacity_metaConsCheck.csv", row.names = F)
write.csv(univarRes, file = "test_consensus/vivacity_metaConsCheck_univarRes.csv", row.names = F)
