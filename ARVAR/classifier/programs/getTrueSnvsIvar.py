...
def combIvarDat(path, minFreq, maxFreq, curSamples):
  ...
  for i in filesList:
    ...
    try:
      ...
      sampleName = i.replace(".sorted.bam_snps.tsv", "")
      dfFilt["ExactSamp"] = sampleName
      sampleList = sampleName.split("-")[0:3]
...
ivarDat = combIvarDat(path = "/home/ubuntu/extraVol/Viralrecon/covid/Ludy_metaseq_2023-05-24/output/relaxIvar/",
                      minFreq = 0.02, maxFreq = 1, curSamples = curSamples)

len(pd.unique(ivarDat["Sample"]))
list(ivarDat)
...
trueSnvs.to_csv("combDatLudyTrueSnvs.csv", index = False)
